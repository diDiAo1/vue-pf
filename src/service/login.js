...
// 系统登录连接后台请求
export  const  basicPlatform_login = (options) => Vue.axios.get(
  // vm.requetHost+'/sys/user/login?loginName=admin&password=admin');
 vm.requetHost+'sys/user/login',{params:options});

// 系统退出连接后台请求
export  const  basicPlatform_logout = () => Vue.axios.post(
  vm.requetHost+'sys/user/logout');

  // 修改密码
export  const  basicPlatform_updatePwd = (options) => Vue.axios.get(
